...
class FlexiCubes:
    ...
    def __call__(
        ...
        assert torch.is_tensor(voxelgrid_vertices) and check_tensor(
        ...
        assert torch.is_tensor(scalar_field) and check_tensor(
            scalar_field, (num_vertices,), throw=False
        ), "'scalar_field' should be a tensor of shape (num_vertices,)"
        assert torch.is_tensor(cube_idx) and check_tensor(
        ...
        assert beta is None or (
            torch.is_tensor(beta) and check_tensor(beta, (num_cubes, 12), throw=False)
        ), "'beta' should be a tensor of shape (num_cubes, 12)"
        assert alpha is None or (
            torch.is_tensor(alpha) and check_tensor(alpha, (num_cubes, 8), throw=False)
        ), "'alpha' should be a tensor of shape (num_cubes, 8)"
        assert gamma_f is None or (
            torch.is_tensor(gamma_f)
            and check_tensor(gamma_f, (num_cubes,), throw=False)
